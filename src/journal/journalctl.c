...
static int vl_server(void) {
        ...
        r = sd_varlink_server_add_interface(varlink_server, &vl_interface_io_systemd_Journalctl);
        if (r < 0)
                return log_error_errno(r, "Failed to add Varlink interface: %m");

        r = sd_varlink_server_bind_method(varlink_server, "io.systemd.Journalctl.GetEntries", vl_method_get_entries);
        if (r < 0)
