...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] COMMAND ...\n\n"
               ...
               "                         Serve a command via varlink protocol upgrade\n"
...
static int varlink_server_add_interface_from_method(sd_varlink_server *s, const char *method, sd_varlink_interface **ret_interface) {
        ...
        _cleanup_free_ char *idl_text = strjoin("interface ", interface_name, "\nmethod ", dot + 1, " () -> ()");
        if (!idl_text)
                return log_oom();
...
static int method_serve_upgrade(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        r = pidref_safe_fork("(serve)", FORK_RESET_SIGNALS|FORK_DETACH|FORK_LOG, /* ret_pid= */ NULL);
        if (r < 0)
                return log_error_errno(r, "Failed to fork: %m");
        if (r == 0) {
                r = rearrange_stdio(input_fd, output_fd, STDERR_FILENO);
                if (r < 0) {
                        log_error_errno(r, "Failed to rearrange stdio: %m");
                        _exit(EXIT_FAILURE);
                }
...
static int verb_serve(int argc, char *argv[], uintptr_t _data, void *userdata) {
        ...
        description = strjoin("serve:", method);
        if (!description)
                return log_oom();
