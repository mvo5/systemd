...
static int varlink_call_and_upgrade(const char *url, const char *method, sd_json_variant *parameters, char **exec_cmdline) {
        ...
        if (!strv_isempty(exec_cmdline)) {
                ...
                /* --exec mode: place the upgraded socket on stdin/stdout so that the child process can
                 * just read/write naturally. This mirrors regular --exec which places the reply on stdin. */
                (void) sd_notify(/* unset_environment= */ false, "READY=1");

                log_close();
                log_set_open_when_needed(true);

                int keep_fds[] = { TAKE_FD(fd) };
                r = close_all_fds(keep_fds, ELEMENTSOF(keep_fds));
                if (r < 0) {
